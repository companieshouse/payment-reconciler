...
func TestUnitGetRefundsCSV(t *testing.T) {
	...
	Convey("Subject: Success", t, func() {
		...
		Convey("Given refunds data is successfully fetched", func() {
			...
			mockDao.EXPECT().GetRefundsData(&reconciliationMetaData).Return(refunds, nil).Times(1)
			...
			Convey("Then no errors are returned", func() {
				...
				Convey("And a CSV is successfully constructed", func() {
					...
					So(refundsCSV.FileName, ShouldEqual, expectedRefundsFileNamePrefix+reconciliationMetaData.ReconciliationDate+expectedCSVFileSuffix)
				})
			})
		})
	})

	Convey("Subject: Failure to retrieve refunds data", t, func() {

		mockDao := dao.NewMockDAO(mockCtrl)

		svc := createMockService(&cfg, mockDao)

		Convey("Given an error when fetching refunds data", func() {

			var refunds models.RefundsList
			mockDao.EXPECT().GetRefundsData(&reconciliationMetaData).Return(refunds, errors.New("failure to fetch transactions data")).Times(1)
